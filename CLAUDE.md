...
## The Action column
Two columns in the **Scores** group — `Type` (ETF | Established | Early) and `Action` (Sell Immediately … Strong Buy) — plus a per-cell tooltip naming **the one rule that fired**. This is the second build; the first (a points model) was reverted, and the owner's constraints for the redo are load-bearing: **no custom composites by default** (Momentum, Quality — `use_momentum` / `use_quality` restore the original gates), no MA Cross, loss-avoidance first.
