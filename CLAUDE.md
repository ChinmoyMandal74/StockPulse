...
- **Profiles.** The **house** profile is one row in `action_rules` (`{preset, overrides}` — overrides being ONLY the diff, so improved defaults still arrive), admin-edited via `PUT /api/action-rules`, which **rejects** invalid profiles with the validator's named reasons (inverted bands, Early stop looser than Established…) and reports unknown keys. `/api/stocks` scores rows **on the way out** (~1ms), so a house edit shows on the next load, not after the nightly refresh, and pre-feature snapshots still carry the column. Each user may keep **one personal profile** in `prefs` (`actionSource`/`actionPreset`/`actionOverrides`), applied in the browser by `applyLens()`, columns tinted while active (`body.rulesed`), CSV header naming whichever profile produced it. The server, the email and the assistant always see the house set.
- **The Rules picker** edits whichever profile you are viewing (house = admin-only). Presets are Conservative / Balanced / Aggressive as diffs; the advanced surface is deliberately a **JSON overrides textarea** validated on save — the reverted build's 70 numeric inputs in a dropdown is the thing not to rebuild. Every key and default is documented in **Help §12**, which also amends §1's old "never offers a verdict" line.
