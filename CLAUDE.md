...
- **`Past Mom. (2W)` and `Mom. Delta (2W)` are formulas over the Momentum column, not more stored values.** The sheet is newest-first, so the score a fortnight ago is the same column ten rows down — the identical offset `momentum_deltas` lags by and `PAST_DEFAULT` means, which is why the workbook agrees with the screener exactly rather than approximately. Verified across **310 rows against the view, worst gap 0.000000**; CRWD reads Momentum 72.6, Past 67.8, Delta +4.8 on both.
