...
- **One fixed rule set, and that is a decision, not a gap.** The Balanced defaults resolve once at boot (`ACTION_CFG` in server.js) and score every row on the way out of `/api/stocks` (~1ms), so pre-feature snapshots still carry the columns. The full configurability layer — house profile in an `action_rules` table, `GET/PUT /api/action-rules`, per-user profiles in prefs, a Rules picker with presets and a JSON editor — **was built, verified end-to-end, and then removed at the owner's instruction on 2026-09-11** ("I don't need any House Rules, my rules and Balanced, Conservative etc at the moment"). The engine still takes a config and `PRESETS` still exist for the offline tools, so bringing it back is wiring, not a rewrite; the removed version is one commit back if wanted. The `action_rules` table was dropped from Turso again.
