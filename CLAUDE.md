...
The workbook: four sheets — **Bars**, **Factors**, **Score**, **Signal** — in which every cell is a live formula over the daily bars. Change a close price and the score moves, because the workbook is the calculation rather than a picture of it.

- **The Bars sheet's `Momentum` column is the one stored value in the workbook**, read from `momentum_history` and aligned by date. Every other cell recalculates; this one cannot, because the score at an earlier date needs a year of run-up before it and reproducing that down 320 rows would mean 320 copies of the Factors sheet. The sheet's own header note says so, and the current score stays live on Score, which is the one anybody actually checks. Verified: 320 values, no formulas, row 5 reads 72.6 against the 72.6 the app reports for CRWD.
- **`Return 2W %` and `Next 2W Return %` sit beside them, as formulas over Close.** The sheet is newest-first, so **the past is *down* the rows and the future is *up* them** — `(C{R}-C{R+10})` against `(C{R-10}-C{R})`. Getting that backwards would silently invert the whole question a backtest is asking. Verified against the view: 310 rows each, worst gap 0.000000000, and row 15's Next-2W formula is byte-identical to row 5's Return-2W, which is the `fwd_ret_2w[t] = ret_2w[t+10]` identity.
- **`Past Mom.` and `Mom. Delta` are carried for all five horizons** — 1W/2W/1M/3M/6M, paired by horizon (Past 1W, Delta 1W, Past 2W, …) because that is the pairing the screener's Past picker expresses. **`HORIZONS` is derived from `PAST_PERIODS` in `screens.js`**, the module that defines that picker, so a workbook cannot disagree with the dropdown it is named after; adding a sixth period there adds two columns here and nothing else.
- **They are formulas over the Momentum column, not more stored values.** The sheet is newest-first, so the score N sessions ago is the same column N rows down — the identical offsets `momentum_deltas` lags by, which is why the workbook agrees with the screener exactly rather than approximately. Verified: all five horizons match the app's `pastMomentum` for CRWD to the digit (65.6 / 67.8 / 77.9 / 52.4 / 21.8), and the 2W delta column matches the view across **310 rows, worst gap 0.000000000**.
- **Both test the referenced cell for a blank rather than trusting it.** An empty cell is 0 in Excel, so a gap in the stored history would otherwise print a past score of zero and a delta the full size of today's. The last ten rows have no fortnight behind them and are left empty — checked at the boundary: formulas through row 314, empty from 315.
- **`momentumMap()` is exported and used by both callers.** The CLI and `/api/model` are verified byte-identical, which only holds while they assemble their inputs the same way — re-checked after adding the column, SHA-256 equal.
- **The Signal sheet carries a real Excel scatter chart** of `Mom. Delta (2W)` against `Next 2W Return %` — the relationship `/signal` draws, in the workbook. Its statistics are formulas too (`CORREL`, `RSQ`, `SLOPE`, and `SUMPRODUCT` for the rates), so it is not a screenshot of a finding; edit a close price and the correlation moves with the cloud.
  - **A chart is four more parts in the zip** — `charts/chart1.xml`, `drawings/drawing1.xml`, a relationship apiece, and two content-type overrides — plus `<drawing r:id>` on the sheet, which **must follow `sheetData`**: the schema fixes the order and Excel silently drops the chart if it does not. The worksheet element gained the `r:` namespace for it.
  - **Every count is gated on `ISNUMBER`.** A blank from `IF(…,"")` is *text*, and text compares greater than any number in Excel, so `(Q>=0)` alone counts the empty rows at both ends as positive. That would have inflated the hit rate and left the lift quietly wrong.
  - **Series refs are looked up from the header row, never hardcoded.** `colOf('Mom. Delta (2W)')` — adding a horizon shifts every column after it, and a chart pointed at a literal `Q` would plot the wrong series rather than fail.
  - **The chart's numbers will not match the page's**, and should not: the workbook holds ~300 sessions and the page the whole history. CRWD reads **r 0.187 over 300** against **0.032 over 2,235** — the small-sample wander the page's caveats warn about, visible in one file. Verified by replaying the cell logic in JS and against the database for the same window: r 0.187, mean next 2.32%, both exact.
  - Validated with a real xlsx parser, not just by eye: it opens, and reads back as a `ScatterChart` with the right refs, a linear trendline with r² displayed, and circle markers.
...
- **119 is a hard ceiling** — Twelve Data rejects a batched `time_series` over 120 symbols and SPY is appended as the benchmark. At 93 there are 26 slots free; at 116 there were three. Past the ceiling the call needs chunking.
...
- **The view is dropped and recreated on every init**, so its definition can never lag the code documenting it. A view carries no data, so this is free.
...
- **The momentum pane sits between the price and RSI** (`MOM_H = 52`), **on by default**. It is the score the whole screener sorts on, so on the page for one stock it is the second thing worth seeing after the price rather than an indicator you go looking for — RSI and volume describe how a move happened, this says what the move is currently worth. Its series is **read from `momentum_history`, not derived in the browser**: momentum needs a year of run-up the chart window does not carry, so `/api/history` returns a `momentum` array already aligned to the bar dates by date lookup, and a gap in it breaks the path rather than joining across it.
...
- **Momentum score slope** — the least-squares slope of the score, in points per session. Knobs: fit window, skip, smoothing. **No volatility normaliser, and that is not an oversight**: the score has been on a fixed absolute scale since September, so 0.4 points a session means the same thing for every stock — which it would not while the score was a percentile. A direct payoff from that change.
  - It is the honest version of `Mom. Delta`, which reads two observations and discards everything between them. **A fit window of 2 reproduces the delta exactly** — verified, `[…,10,14]` gives 4 — so the delta is the degenerate case of this.
  ...
  - The score is **aligned to the bar dates by lookup, never by position** — `momentum_history` has no row until a symbol has `MIN_BARS` of run-up, so its dates are a subset and zipping the arrays would offset the whole series. Verified: 273 leading nulls for MU, exactly `MIN_BARS` − 1.
- **RSI** — Wilder's plain reading, not the sub-score the model derives from it. Knobs: period, skip, smoothing. Verified against `Momentum.rsiSeriesAt` at **gap 0.00e+0**, and against the screener's own RSI column (61.1603 both).
  - **The indicator is disabled when a symbol has no stored score.** That is not defensive dead code: `readMomentum()` filters on `MODEL_VERSION`, so the moment the model is bumped every symbol has zero scores until the backfill finishes. Verified by simulating exactly that — the option greys out, velocity keeps working, and forcing the selection explains itself rather than drawing a blank chart.
...
- **Coverage without a schedule**: `topUpNews()` rides every refresh (each cron round included), topping up the `NEWS_TOPUP_PER_REFRESH` (12) stalest symbols, never-fetched first — the nightly job's ~20 rounds cycle the whole universe in one night. Fire-and-forget; a failed news write never fails a refresh (the bars rule). `GET /api/news?symbol=` is stored-or-fetch with a 6-hour TTL so a visited stock page stays fresh regardless; on a provider failure it serves the stored set, stale beating nothing.
