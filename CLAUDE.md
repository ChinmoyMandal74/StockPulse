...
- **The published long-only formulation does *not* beat holding, on this universe.** Set as specified — 252-day lookback, mandatory 200-day filter, 1× cap, daily, 20-day vol — at matched volatility and 5 bps: CAGR **+9.9%** against hold's +18.6%, Sharpe **0.90 against 0.95**, max drawdown **−16.0% against −43.4%**. Credit 4% on the cash it holds and subtract the same 4% from both Sharpes and it is **0.66 against 0.75**, losing in *both* halves (0.55 vs 0.61 pre-2020, 0.86 vs 0.96 after). What it buys is the drawdown, not the ratio. The `Set the published long-only rule` link on the page puts all five knobs there in one click.
- **Daily rebalancing is only optimal below about 3 bps.** At matched volatility the spec rule turns over **162% of capital a month** against 44% monthly. Measured crossovers: daily leads weekly to **3.15 bps**, weekly leads monthly to **12.86 bps**, monthly above that. At 20 bps daily gives back **4.3 points of CAGR** against monthly. Research quoting a daily-rebalanced vol-scaled rule at zero cost is quoting the most flattering version of the most expensive setting — which is why cost and cash yield are both on screen, since they pull opposite ways and omitting both is not neutral.
