...
- **Fundamentals that moved** — *Refresh all only*, since only a Refresh all re-pulls the profile cache; a plain Refresh would be comparing a number with itself. Each symbol's row in `fundamentals_history` is compared against **the previous recorded set**, which is not "yesterday": a row exists only for days a Refresh all actually ran, so after a quiet stretch the comparison reaches further back. The email prints that date, because a change "since yesterday" that is really since last Thursday is a lie told by omission. `readFundamentalsPair(day)` fetches both sets in one round trip — measured at **48ms** against Turso.
  ...
  - **A symbol with no earlier row is counted, not compared** — it is new to the archive, not a company that changed overnight. The night the 33 faders were added, the email would have said "33 recorded for the first time, so nothing to compare."
...
| **RSI**, 40 settings, cross-sectional at one month | **Best top-decile t: 1.76.** Pre-2020 t 0.1, post-2020 t 2.2. On a single stock the deciles are visibly non-flat while the correlation reads 0.044 — which is the lesson, not a finding |
...
- **One file per universe, plus an index (`strategy-index.json`).** The grid is 192 variants and every universe's copy of it is 12.7 MB — a multi-megabyte download to read one entry in a dropdown. The page fetches the index, then the chosen universe, and caches each. The largest is 957 KB raw, **230 KB over the wire**, and only one is ever fetched.
...
- **What the grid says so far: nothing, for any of the three.** RSI's best top-decile t is **1.76** across 40 settings — and note that is the *cross-sectional* view, where the per-stock decile shape above largely washes out.
- **What the grid says: nothing, for either of the first two.** Velocity's best top-decile t over the full period is **1.73** across 144 settings; the slope's is **0.80** across 48. With that many tests anything under about 3 is noise, and the split is the familiar one — the slope reads pre-2020 t −1.3, post-2020 t +2.0. **A better-estimated version of a flat thing is still flat**, which is the useful answer: the delta's problem was never its estimator.
