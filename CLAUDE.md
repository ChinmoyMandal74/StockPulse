...
- **Five fixed profiles, no custom set — the picker came back on 2026-09-12, lighter than the original.** The `Rules` picker (`#actPicker`, beside Weights) chooses Balanced · Trend Rider · Aggressive · Max Risk · Dip Buyer; the owner explicitly ruled out a custom option, so a verdict is always explainable by one profile name plus the ladder panel, which draws whichever profile is active (`actionCfg()` replaced the fixed panel cfg). **Client-side re-score only**: `scoreActionRows()` re-stamps the same seven fields the server stamps, via `ActionRules.apply()` on every load and on a profile switch — no `action_rules` table, no routes, no editor this time. The choice is one validated id in prefs (`action`, allowlisted in the PUT like `past`); the snapshot, nightly report and assistant stay on **Balanced** (`ACTION_CFG` in server.js untouched), the six columns tint while off Balanced (`body.act-lensed`, the weight-lens convention), and the CSV names the profile in the Action headers and filename. Trend Rider is entry-side aggression only (chase caps loose, stops Balanced); Max Risk is Aggressive turned further but still never below the 200D; Dip Buyer alone flips `trend_gate` (stops stay Balanced, and Help says the research log does not endorse the mode). All presets validate, ride the sweep (now 24,000 evaluations across 12 profiles), and have signature tests pinning what each changes *and* what it deliberately does not. **The first build's history**: a fuller layer (house profile in a DB table, per-user diffs, a JSON editor) was built, verified end-to-end, and removed at the owner's instruction on 2026-09-11; the `action_rules` table stays dropped.
